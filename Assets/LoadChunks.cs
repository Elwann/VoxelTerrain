...
public class LoadChunks : MonoBehaviour
{
    ...
    void FindChunksToLoad()
    {
        ...
        WorldPos playerPos = new WorldPos(
            Mathf.FloorToInt(transform.position.x / World.chunkSize) * World.chunkSize,
            Mathf.FloorToInt(transform.position.y / World.chunkSize) * World.chunkSize,
            Mathf.FloorToInt(transform.position.z / World.chunkSize) * World.chunkSize
        );
        ...
        if (buildList.Count == 0)
        {
            ...
            for (int i = 0; i < chunkPositions.Length; i++)
            {
                ...
                //Debug.Log(newChunkPos.x + ", " + newChunkPos.y + ", " + newChunkPos.z);

                //If the chunk already exists and it's already
                //rendered or in queue to be rendered continue
                if (newChunk != null && (newChunk.Complete() || buildList.Contains(newChunkPos)))
                    continue;

                //load a column of chunks in this position
                for (int y = 0; y < 1; y++)
                {
                    buildList.Add(new WorldPos(newChunkPos.x, y * World.chunkSize, newChunkPos.z));
                }
